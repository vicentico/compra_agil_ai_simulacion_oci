...
| Estado global | **FASE 0 aprobada · FASE 1-7 (Docker infra, Observability, Identity & security, Procurement domain, ChileCompra integration, Incremental synchronization, Document storage) implementadas** (2026-08-17). Próxima: FASE 8 — Document intelligence. |
...
│   ├── modules/documents/Ppip.DocumentIntelligence.{Domain,Application,Infrastructure}/  ← FASE 7 (Document + DocumentDownloadOrchestrator + HttpAttachmentDownloader/SsrfSafeConnect/MinioObjectStorage) — nuevo bounded context, mismo patrón de 3 capas
...
├── tests/Ppip.DocumentIntelligence.Domain.Tests/           ← FASE 7, nuevo proyecto (23 tests)
├── tests/Ppip.DocumentIntelligence.Application.Tests/      ← FASE 7, nuevo proyecto — DocumentDownloadOrchestrator con dobles en memoria (9 tests)
├── tests/Ppip.DocumentIntelligence.Infrastructure.Tests/   ← FASE 7, nuevo proyecto — Mongo/MinIO reales vía Testcontainers + SSRF contra la pila de red real (12 tests)
...
---
*Fin del handover. El asistente entrante debe confirmar lectura de README + ROADMAP + `infrastructure/docker/README.md` y proceder con la tarea inmediata (§5.3, FASE 8) siguiendo el formato de respuesta §49.*
