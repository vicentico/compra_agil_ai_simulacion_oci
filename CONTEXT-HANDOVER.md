...
| Estado global | **FASE 0 aprobada · FASE 1-9 (Docker infra, Observability, Identity & security, Procurement domain, ChileCompra integration, Incremental synchronization, Document storage, Document intelligence, RAG) implementadas** (2026-08-19). Próxima: FASE 10 — AI analysis. |
...
│   ├── modules/documents/Ppip.DocumentIntelligence.{Domain,Application,Infrastructure}/  ← FASE 7 (Document + DocumentDownloadOrchestrator + HttpAttachmentDownloader/SsrfSafeConnect/MinioObjectStorage) + FASE 8 (ClassificationPolicy/PdfPigExtractor/DocumentProcessingOrchestrator/ChunkingService/MockOcrService+TesseractOcrService) + FASE 9 (DocumentChunk.EmbeddingId, IDocumentRepository.FindByCompraAsync, DocumentMongoDatabaseProvider) — nuevo bounded context, mismo patrón de 3 capas
│   ├── modules/knowledge/Ppip.Knowledge.{Domain,Application,Infrastructure}/  ← FASE 9 (Embedding/AIExecution/AnswerType + EmbeddingIndexer/RagQueryOrchestrator + MockEmbeddingProvider+OllamaEmbeddingProvider/MockLlmProvider+OllamaLlmProvider/QdrantVectorIndex) — nuevo bounded context; Application referencia directo Domain/Ports de DocumentIntelligence y Procurement (ADR-012)
│   ├── services/Ppip.PlatformApi/     ← FASE 1, instrumentado en FASE 2+3 → FASE 9 (primer endpoint de negocio real: POST /api/rag/{compraId}/query, compone Procurement+DocumentIntelligence+Knowledge)
│   ├── workers/Ppip.AiWorker/         ← FASE 1, instrumentado en FASE 2+3, sin cambios desde entonces
│   ├── workers/Ppip.SyncWorker/       ← FASE 1 (esqueleto) → FASE 6 (SyncSchedulerWorker real, ya no es el heartbeat placeholder)
│   ├── workers/Ppip.DocumentWorker/   ← FASE 1 (esqueleto, heartbeat se mantiene) → FASE 7 (composition root de UC-003 pasos 1-3) → FASE 9 (+ EmbeddingIndexer, POST /internal/documents/{id}/index)
│   └── apps/frontend/                ← FASE 1 (Angular 20.3)
├── docs/07-events/schemas/           ← FASE 4: JSON Schema de CompraAgilDetected/Updated.v1 + FASE 8: DocumentDetected/Downloaded/Extracted/OcrCompleted/Chunked.v1 + FASE 9: EmbeddingCreated.v1
├── tests/Ppip.BuildingBlocks.Observability.Tests/  ← FASE 2, xUnit (4 tests, CorrelationIdMiddleware)
├── tests/Ppip.PlatformApi.Tests/                   ← FASE 3, xUnit (12 tests, RBAC vs Keycloak real/Testcontainers) — FASE 9: sigue en 12, ahora también smoke-testea que el contenedor DI con Procurement+DocumentIntelligence+Knowledge combinados arranca correcto
...
├── tests/Ppip.ArchitectureTests/                   ← FASE 4 (4) + FASE 7 (+2) + FASE 9 (+2: Knowledge.Domain sin infra, Knowledge.Application sin Infrastructure de ningún módulo) = 8 tests
├── tests/Ppip.Events.Contracts.Tests/              ← FASE 4 (4) + FASE 6 (+2) + FASE 8 (+5) + FASE 9 (+1: EmbeddingCreated.v1) = 12 tests
...
├── tests/Ppip.DocumentIntelligence.Domain.Tests/           ← FASE 7 (23) + FASE 8 (+29) + FASE 9 (+3: EmbeddingId/MarkEmbedded) = 55 tests
...
├── tests/Ppip.Knowledge.Domain.Tests/               ← FASE 9, nuevo proyecto — Embedding, AIExecution (10 tests)
├── tests/Ppip.Knowledge.Application.Tests/          ← FASE 9, nuevo proyecto — EmbeddingIndexer, RagQueryOrchestrator con dobles en memoria, cubre UC-005 A1/A2/A3 (17 tests)
├── tests/Ppip.Knowledge.Infrastructure.Tests/       ← FASE 9, nuevo proyecto — MockEmbeddingProvider/MockLlmProvider + Mongo/Qdrant reales vía Testcontainers (14 tests)
...
11. **FASE 9 — RAG: IMPLEMENTADA (2026-08-19). UC-005 completo** (Consultar RAG) + docs/09 etapas 10-11 (Embedding/Indexing). Nuevo bounded context `Ppip.Knowledge.{Domain,Application,Infrastructure}` en `src/modules/knowledge/`, y **primer endpoint de negocio real de `Ppip.PlatformApi`** — hasta FASE 8 ese servicio era deliberadamente un esqueleto (topología+observabilidad+RBAC); `POST /api/rag/{compraId}/query` es la primera vez que compone Procurement+DocumentIntelligence+Knowledge en un mismo proceso.

**OQ-03 cerrada:** nomic-embed-text vía Ollama, 768 dimensiones, colección Qdrant `chunks_v1` (coseno, índice de payload por `compraAgilId`). Dominio nuevo: `Embedding` (referencia al vector, nunca el vector en sí), `AIExecution` (auditoría de toda invocación IA — se registra en TODA consulta, exista o no evidencia, UC-005 paso 8 literal), `AnswerType`. `DocumentChunk` (FASE 8) gana `EmbeddingId`/`MarkEmbedded`. `ILlmProvider` simplifica deliberadamente el contrato literal de ADR-007 (texto crudo + usage, no JSON estructurado validado por schema).

**Cross-context por diseño (ADR-012):** `Ppip.Knowledge.Application` referencia directo `Ppip.DocumentIntelligence.Domain.Ports.IDocumentChunkRepository` (Qdrant no guarda texto) y `Ppip.Procurement.Domain.Ports.ICompraAgilRepository` (precondición UC-005). Blindado por architecture test.

**UC-005 A1/A2/A3 con comportamientos deliberadamente distintos:** A1 corta antes del LLM (→ "Información no encontrada...", `AIExecution` igual registrado). A2 (Qdrant/embeddings caídos) → 503 explícito, sin fallback. A3 (LLM caído) → 200 degradado a chunks-como-búsqueda, sin síntesis, nunca error.

**Un bug real de diseño encontrado y corregido antes de ejecutarse:** `AddProcurementPersistence()`/`AddDocumentPersistence()` registraban `IMongoDatabase` directo en el contenedor DI — inofensivo mientras cada módulo vivía en su propio worker, pero `Ppip.PlatformApi` es el primer proceso que combina los tres, y la combinación habría hecho que los tres resolvieran silenciosamente la MISMA base de datos (la última registrada gana). Corregido con un wrapper dedicado por módulo (`ProcurementMongoDatabaseProvider`/`DocumentMongoDatabaseProvider`/`KnowledgeMongoDatabaseProvider`) antes de escribir el endpoint. Regresión cubierta por `Ppip.PlatformApi.Tests` (el contenedor DI combinado arranca correcto contra Keycloak real).

**Validado contra infra real:** MongoDB y **Qdrant** (creación de colección, upsert, búsqueda filtrada por `compraAgilId` — incluye la prueba de que una compra distinta jamás ve evidencia ajena) vía Testcontainers. `MockEmbeddingProvider`/`MockLlmProvider` (determinísticos) son el default de producción; `OllamaEmbeddingProvider`/`OllamaLlmProvider` implementados (REST real contra Ollama) pero **sin ejecutar contra un modelo Ollama real descargado en esta sesión** — mismo criterio que `TesseractOcrService` (FASE 8).

**47 tests nuevos, todos en verde:** Knowledge.Domain 10, Knowledge.Application 17 (cubre A1/A2/A3 + postcondición "ninguna mutación de dominio"), Knowledge.Infrastructure 14 (incluye Qdrant/Mongo reales), DocumentIntelligence.Domain +3, ArchitectureTests +2, Events.Contracts +1. **Fuera de esta fase, explícito:** keyword search/RRF/reranking (SHOULD, se difieren con datos según lo previsto); dataset `/evaluation` (bloqueado transitivamente por OQ-02); filtros `documentId`/`section` del contrato de ejemplo; 409 "compra sin índice construido" (cae en el mismo camino que A1); consumidor RabbitMQ real de `EmbeddingCreated.v1` (AI Worker, FASE 10); test HTTP end-to-end del endpoint contra Mongo+Qdrant+Ollama reales combinados (validado por partes, ver arriba). Detalle completo en `docs/ROADMAP.md` nota de cierre de FASE 9.
...
**FASE 10 — AI analysis:** análisis IA estructurado con schemas y evaluación (UC-004 parte 1, FR-024/025). Primer consumidor real de `AIExecution` para análisis completo de compra (no solo consulta RAG), y primer uso de `AnswerType`/`ILlmProvider` (ambos ya en `Ppip.Knowledge.Domain` desde FASE 9, deliberadamente diseñados para reusarse acá) más allá del caso de uso RAG. Candidato natural para revisar si el contrato simplificado de `ILlmProvider` (texto crudo, FASE 9) sigue alcanzando o si hace falta acercarse más al `CompleteStructuredAsync` literal de ADR-007 (JSON validado por schema).

Nota operativa para retomar el stack: `make up && make up-obs && make smoke && make smoke-obs`. Traefik puede fallar a bindear el puerto 80 si el host ya lo tiene ocupado (pasó en varias sesiones, por otro proceso ajeno al proyecto) — no es un bug del compose; en ese caso los servicios de app siguen accesibles vía `docker exec <container> curl http://localhost:8080/...` o publicando temporalmente otro puerto. Para probar auth manualmente sin Traefik, agregar los headers `X-Forwarded-Host/Proto/Port` que Traefik normalmente añade (ver `infrastructure/docker/README.md`). Para dominio/aplicación/infra de Procurement, Document Intelligence y Knowledge no hace falta levantar nada para los tests unitarios/de aplicación (`Ppip.*.Domain.Tests`, `Ppip.*.Application.Tests`, `Ppip.ArchitectureTests` corren sin Docker); los `*.Infrastructure.Tests` sí necesitan Docker (Testcontainers para Mongo/Redis/MinIO/Qdrant). **Docker Desktop y el SDK de .NET 10 están disponibles y funcionando en este entorno** (confirmado en las sesiones de FASE 6-9: `docker version`/`dotnet --version` responden, Testcontainers descarga y corre contenedores reales sin problema — incluye MongoDB, Redis, MinIO y Qdrant).

### 5.4 Riesgos y cuestiones abiertas a vigilar
RSK-02 (alucinaciones — mitigación en cada capa), RSK-05 (sobreingeniería — resistir), RSK-12 (alcance MUST grande — gates), RSK-14 (cold start del score), **OQ-02 (descarga de adjuntos — investigada a fondo en FASE 7-8, sigue abierta; FASE 9 siguió usando chunks de PDFs sintéticos, FASE 10 previsiblemente también)**, OQ-06/ASM-08 (revisión legal formal de términos de uso, sigue pendiente), OQ-09 (taxonomía de rubros para matching — `codigo_producto` parece UNSPSC, sin confirmar, F12), OQ-11 (FASE 6: `EstadoCompra` no modela `cancelada` — compras así quedan en cuarentena hasta que aparezca un caso real). **OQ-03 cerrada en FASE 9** (nomic-embed-text/768).
...
---
*Fin del handover. El asistente entrante debe confirmar lectura de README + ROADMAP + `infrastructure/docker/README.md` y proceder con la tarea inmediata (§5.3, FASE 10) siguiendo el formato de respuesta §49.*
