...
**FASE 0 aprobada** · **FASE 1-9 implementadas** (Docker infra, Observability, Identity & security, Procurement domain, ChileCompra integration, Incremental synchronization, Document storage, Document intelligence, RAG) (2026-08-19) — próxima: **FASE 10 — AI analysis**.
Especificación completa en [docs/](docs/); infraestructura y esqueletos ejecutables en `infrastructure/docker/` y `src/`. UC-001 (sincronización incremental) completo e idempotente: `Ppip.Procurement.Domain` (49 tests) + `Ppip.Procurement.Application` (`SyncOrchestrator`, 19 tests) + `Ppip.Procurement.Infrastructure` (cliente ChileCompra + persistencia Mongo + lock Redis + outbox dispatcher RabbitMQ, 31 tests) + `Ppip.SyncWorker`. UC-003 completo (descarga validada anti-SSRF/MinIO/hash + clasificación/extracción/OCR/chunking semántico + embedding/indexing en Qdrant): `Ppip.DocumentIntelligence.Domain` (55 tests) + `Ppip.DocumentIntelligence.Application` (24 tests) + `Ppip.DocumentIntelligence.Infrastructure` (22 tests, PdfPig + SSRF contra la pila de red real + Mongo/MinIO reales) + `Ppip.DocumentWorker` — pendiente de conectar a una fuente real de adjuntos (OQ-02 abierta, la API de ChileCompra no expone descarga). UC-005 (Consultar RAG, primer endpoint de negocio real de `Ppip.PlatformApi`): `Ppip.Knowledge.Domain` (10 tests) + `Ppip.Knowledge.Application` (`RagQueryOrchestrator`, `EmbeddingIndexer`, 17 tests) + `Ppip.Knowledge.Infrastructure` (14 tests, Qdrant/Mongo reales vía Testcontainers) — modelo de embeddings nomic-embed-text/768 (OQ-03 cerrada), proveedores Mock por defecto (Ollama implementado pero sin modelo real descargado en esta sesión). Todo validado con Testcontainers (Mongo/Redis/MinIO/Qdrant reales, Docker disponible en el entorno) y contra un Keycloak real para el arranque del contenedor DI combinado. Ver [docs/ROADMAP.md](docs/ROADMAP.md) (incluye qué se validó en cada fase).
