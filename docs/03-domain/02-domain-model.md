...
## Estado de implementación (actualizado FASE 9, 2026-08-19)
...
**Document Intelligence** (FASE 7-9: UC-003 pasos 1-9, descarga+storage+clasificación+extracción+OCR+chunking): `src/modules/documents/Ppip.DocumentIntelligence.Domain` — `Document` (Identidad `DocumentId`, `DocumentStage`: descarga/storage, FASE 7), `DocumentVersion` (VOs `Sha256Hash`/`StorageRef`; gana en FASE 8 `DocumentProcessingStage`, `DocumentClass`, lista de `DocumentPage` — máquina de estados independiente de `DocumentStage` porque versiona "qué se hizo con el binario", no "cómo se obtuvo el binario"), `DocumentPage` (texto, método de extracción, densidad, confianza OCR), `DocumentChunk` (entidad propia, no anidada — colección `document_chunks`; gana en FASE 9 `EmbeddingId`/`MarkEmbedded`), `Policies/UrlAllowlistPolicy`, `Policies/PdfMagicBytes`, `Policies/ClassificationPolicy` — 55 tests xUnit, sin infraestructura (NFR-013, puertos en `Domain/Ports`). **Todavía no implementado:** `ExtractionReviewTask` (HITL, FR-053/054, FASE 8 propuesta de plataforma — llega cuando se implemente ese flujo, no antes).

**Knowledge/RAG** (FASE 9: UC-005 completo, docs/09 etapas 10-11 + query): `src/modules/knowledge/Ppip.Knowledge.Domain` — `Embedding` (referencia al vector Qdrant, nunca el vector en sí), `AIExecution` (auditoría de toda invocación IA, compartida a futuro con AIAnalysis/FASE 10), `AnswerType` (Fact/Inference/Recommendation/Unknown, ídem), puertos `IEmbeddingProvider`/`IVectorIndex`/`ILlmProvider` (`ILlmProvider` es una simplificación deliberada del contrato literal de ADR-007 — texto crudo + usage, no JSON estructurado validado por schema; ver docs/10-rag/01 nota de cierre) — 10 tests xUnit, sin infraestructura. Cross-context: `Ppip.Knowledge.Application` referencia directamente `Ppip.DocumentIntelligence.Domain.Ports.IDocumentChunkRepository` (resolver texto tras la búsqueda vectorial, que Qdrant no guarda) y `Ppip.Procurement.Domain.Ports.ICompraAgilRepository` (validar precondición de UC-005) — desviación consciente de la pureza de bounded context, justificada por el propio ADR-012 ("Revisión de boundaries al cierre de FASE 10/15/18").

Los demás contextos (Proposal Management, Compliance, Audit) todavía no tienen dominio implementado — se construyen en sus fases (12+, 14, 15).
